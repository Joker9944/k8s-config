...
Published: `gotify-custom`, `jinja-cli`, `postgresql-client`. (`base` is built but not published — it only exists to be layered on.)
...
Two regex managers cover the rest, both reading `*.cue`:

| Matches                                           | Needs a comment?                 |
| ------------------------------------------------- | -------------------------------- |
