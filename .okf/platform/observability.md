...
status: stable
generated: { by: claude-code/opus-5, at: 2026-09-10T17:10:00Z }
---
...
kanidm and audiobookshelf still parse in the base config's `loki.process`, and **their `stage.regex` captures are discarded** — no `stage.labels` or `stage.structured_metadata` follows, so `level` and `message` never leave the extracted map. servarr's pipeline does carry a `stage.structured_metadata`, so it is the one workload whose parsed `level` and `unit` reach Loki.
...
Two gates cover these files, failing for different reasons. `alloy fmt -w` runs in pre-commit beside `cue-fmt`, so formatting is canonical and auto-fixed rather than matched by eye — it loops because `alloy fmt` takes at most one file and pre-commit passes the batch. `checks.alloyValidate` owns semantics: it assembles the same union the sidecar delivers and rejects a dangling cross-file reference before it can reach a node.
