...
`spec.verify` is deliberately absent. Artifacts are meant to be cosign-signed and verified here, but **nothing publishes them yet**, and a verify block against unsigned artifacts fails the reconcile. Until the publish workflow exists, level 2 points at tags that do not resolve.
