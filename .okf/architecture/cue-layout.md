...
status: stable
generated: { by: claude-code/fable-5, at: 2026-09-16T00:00:00Z }
...
`#Release.crds` switches on the `install`/`upgrade` block — `crds: CreateReplace` with three remediation retries. It only bites on a chart with a Helm `crds/` directory at a chart root, which is four of the fleet: traefik, redis-operator, kube-prometheus-stack (its `crds` subchart) and nvidia-device-plugin (its node-feature-discovery subchart). cert-manager, cnpg, barman-cloud, volsync and metallb render their CRDs as ordinary templates — `templates/crds/` and a subchart _named_ `crds` both count as templates — so the flag never reaches them. garage is a third shape: it _has_ a CRD, `garagenodes.deuxfleurs.fr`, but the chart ships only RBAC to create it and garage creates it itself at startup for Kubernetes discovery — the live object carries no Helm ownership metadata, so Helm has nothing to adopt. `#Release.postRenderers` passes kustomize patches through to the HelmRelease, for what a chart exposes no knob for — opencloud mounts the private CA bundle that way. `#GitRepo` takes `branch` or `tag` plus an optional `ignore`, because two of the three git-sourced charts pin a tag and ship one directory out of a whole repository.
